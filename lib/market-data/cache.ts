 ...
 *
 * Single owner of the in-memory market-data cache. Holds the Map, the cache
 * primitives, the NYSE-aware dynamic TTL logic, and the market-facing key
 * builders. ETF research originally lived here too — that pipeline was
 * retired, leaving market-data as the sole tenant.
 *
 * Centralising the cache in one module keeps a future Redis migration to a
 * one-file change.
 */
...
// Floor / ceiling values used by the dynamic TTL logic. Exported so that
// tests and callers can reason about freshness guarantees without hard-coding
// magic numbers.
