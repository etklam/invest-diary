  ...
  <div class="space-y-3">
    <LedgerCard v-for="document in documents" :key="document.basename" class="!p-4">
      <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <a :href="href(document.basename)" class="inline-flex min-h-11 shrink-0 items-center justify-center rounded-dt-sm bg-dt-primary-solid px-4 font-semibold text-white"><Icon name="heroicons:arrow-down-tray" class="mr-2 h-4 w-4" />{{ t('secFilings.download') }}</a>
      </div>
    </LedgerCard>
  </div>
</template>
