...
- `http/auth.contract.test.ts`, `http/core.contract.test.ts`, `http/websocket-revocation.contract.test.ts` — skipped by generic Vitest; `npm run test:backend-http:mariadb` starts disposable MariaDB 11.4, verifies native-session and Diary-contract legacy backfills plus rollback/forward paths, boots a built Nitro server, then exercises auth, blog FULLTEXT search, recurring-alert dismissal/scheduler behavior, real Diary CRUD/list/review ownership, validation, conflict, pagination, ID/date/instant and error wire contracts. The WebSocket HTTP contract verifies logout-all, password change and account deletion against real active sockets and checks that other users remain connected. `alert-query.mysql.test.ts` runs the actual scheduler query with 1,000 and 50,000 historical alerts, captures Prisma SQL, and checks the due-range index with MariaDB EXPLAIN/ANALYZE.
...
- `websocket/socket-io.contract.test.ts` — environment-gated in generic Vitest because it binds a loopback listener; `npm run test:socketio` is the required real Socket.IO gate and exercises the production `createSocketServer()` handshake, auth rejection, user room, ping/pong, alert dismissal, expiry, revocation races, and large-ID wire contract. It also launches `malformed-cookie-process.test.ts` in a child process to verify that malformed credentials cannot stop normal HTTP or Socket.IO service.
